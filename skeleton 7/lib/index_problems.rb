...
def toy_time
  ...
  # CREATE INDEX toys_name ON toys(name) gives result == 33 (better than toys_price..)
  execute(<<-SQL)
    EXPLAIN
      SELECT
        toys.price
...
def gold_cat_toys
  ...
  # CREATE INDEX cat_color ON cats(color) gives result_range of 96..538
  execute(<<-SQL)
    EXPLAIN
      SELECT
        toys.name
...
def who_owns_thumbs_up
  ...
  # with the above index on toys_name, plus an index toy_id on cattoys(toy_id), gives range: 4 .. 145
  execute(<<-SQL)
    EXPLAIN
      SELECT
...
def popular_toys
  ...
  # CREATE INDEX cat_id ON cattoys(cat_id) give range of 35.20 .. 32.26
  execute(<<-SQL)
    EXPLAIN
      SELECT
        toys.name,
